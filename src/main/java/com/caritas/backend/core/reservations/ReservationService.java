...
public class ReservationService {
    ...
    public ReservationService(ReservationRepository reservationRepository, UserRepository userRepository,
            HostelRepository hostelRepository, PersonRepository personRepository, ServiceDispatcher serviceDispatcher) {
    ...
    public GetActiveReservationResponse getUserActiveReservation(String id) {
        ...
        ReservationSerialized serializedReservation = activeReservation != null
                ? new ReservationSerialized(activeReservation, user, activeReservation.getHostel(),
                        activeReservation.getPersonReservations(), activeReservation.getServiceReservations(), true,
                        true)
                : null;
    ...
    public GetUserTransportationReservationResponse getUserTransportationReservation(String userId) {
        ServiceReservationEntity serviceReservation = this.reservationRepository
                .findPendingServiceReservationByUserIdAndType(userId, ServiceNames.TRANSPORTATION,
                        ReservationState.ACTIVE, ServiceReservationState.PENDING)
                .orElse(null);

        if (serviceReservation == null) {
            return new GetUserTransportationReservationResponse(null);
        }

        JsonNode response = this.serviceDispatcher.getServiceReservation(ServiceNames.TRANSPORTATION,
                serviceReservation.getExternalReservationId());

        UUID externalServiceId = UUID.fromString(serviceReservation.getExternalReservationId());
        LocalDate orderDate = serviceReservation.getOrderDate();
        Integer count = serviceReservation.getCostCount();
        String hostelName = UtilsJSON.getField("hostelName", response, true).asText();
        String place = UtilsJSON.getField("place", response, true).asText();
        boolean fromHostel = UtilsJSON.getField("fromHostel", response, true).asBoolean();
        LocalTime pickupTime = LocalTime.parse(UtilsJSON.getField("pickupTime", response, true).toString().replace("\"", "").trim());

        return new GetUserTransportationReservationResponse(
                new TransportationReservationSerialized(externalServiceId, orderDate, count, hostelName, place,
                        fromHostel, pickupTime));
    }
